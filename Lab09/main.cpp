...
void entryInsert(netflix_entry *newEnt) {
    ...
    //
    int hashPass =getHashKey(newEnt -> title);
    int count = 0;

    if(!entryHash){
        entryHash[hashPass] = newEnt;
    }
    else{
        newEnt -> next = entryHash[hashPass];
        entryHash[hashPass] = newEnt;
    }
...
bool entryFind(const string &title, netflix_entry &foundEnt, int &ncmp) {
    ...
    //
    if(entryHash[key] == NULL){
        return false;
    }
    else{
        netflix_entry *finding = entryHash[key];
        while(finding != NULL){
            ncmp++;
...
int main() {
    ...
    string userChoice;
    int ncmp;
    netflix_entry netflix_entry1;

    for (;;) {
        //
        // Your input loop goes here.
        cout << "Enter a movie or show title (<return> to quit): ";
        ...
        ncmp = 0;
        if(entryFind(userChoice, netflix_entry1, ncmp )){
            cout << "Comparisons: " << ncmp << endl;
            cout << "Type: " << netflix_entry1.type << endl;
            cout << "Title: " << netflix_entry1.title << endl;
            cout << "Director: " << netflix_entry1.director << endl;
            cout << "Country: " << netflix_entry1.country << endl;
            cout << "Year of Release: " << netflix_entry1.year << endl;
        }
        else{
            cout << "SHUT UP" << endl;
            cout << "Comparisons: " << ncmp << endl;
